...
public class LoginPanel extends UniversalPanel {
    ...
    private final String[] launchEnterText = {
            "Rules:",
            "1. The username must be between 3 - 24 characters long, inclusive.",
            "2. The username can only contain 1-9, A-Z, a-z, dash, underline or spaces.",
            "3. The username must contain at least one letter.",
            "4. Special Symbols such as #%$ are not allowed.",
            "5. The username cannot start or end with dash, underline or spaces.",
            "6. The username cannot be one of the keywords used by the game system."
    };
    public LoginPanel(){
        super();
    }
    protected JComponent[] fetchContent() {
        ...
        JTextField textField = new JTextField(Launcher.IO.Username.MAX_LENGTH);
        return new JComponent[]{(JComponent) Box.createVerticalGlue(), launchEnterTextPrompt, launchEnterTextRequirements[0],
                ...
                textField, (JComponent) Box.createVerticalGlue()};
    }
    ...
    public void recalculate(){
        super.recalculate();
        double referenceGameNameSize = Math.min(getReferenceHeight()*2, getReferenceWidth());
        launchEnterTextPrompt.setFont(new Font(LaunchEssentials.launchEnterUsernameFont, Font.ITALIC, (int)Math.round(referenceGameNameSize/24.0)));
        launchEnterTextPrompt.setBorder(new EmptyBorder((int)Math.round(referenceGameNameSize/150.0)*2, 0, (int)Math.round(referenceGameNameSize/150.0), 0));
